...
// --- NEW VERSIONING CONSTANTS ---
const APP_VERSION = 'v21.15.100'; // You increment this on every new deployment
const VERSION_KEY = 'app_version';
// --- END NEW VERSIONING CONSTANTS ---
...
const AppContent: React.FC = () => {
    ...
    switch (currentView) {
      ...
      default:
        return (
          <motion.div
            key="home"
            className={`relative z-10 pt-16 transition-all duration-300 ${isSidebarOpen ? 'md:pl-64' : 'md:pl-16'}`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.5 }}
          >
                  ...
                  >
                    <div>
                      ...
                      >
                        ...
                        {!searchQuery && !selectedCategory && !selectedAuthor && selectedTags.length === 0 && 'All Posts'}
                      </motion.h2>
                      <motion.p 
  ...
  }, [currentView, isLoading, selectedBlog, isSidebarOpen, blogs, searchQuery, selectedCategory, selectedAuthor, selectedTags, selectedSort, allTags, selectedAuthorName, handleBlogClick, handleLikeBlog, handleClearAllFilters, handleShowEditor, handleTagToggle, handleAuthorSelect, handleCategorySelect, handleSortSelect]);
...
export default function App() {
    // --- KEY CHANGE: WE MOVED THE VERSION CHECK HERE ---
    useEffect(() => {
        const storedVersion = localStorage.getItem(VERSION_KEY);

        if (storedVersion !== APP_VERSION) {
            console.warn(`App version mismatch. Clearing old localStorage data. Old: ${storedVersion}, New: ${APP_VERSION}`);
            localStorage.clear();
            localStorage.setItem(VERSION_KEY, APP_VERSION);
            toast.info("Your application data has been updated to the latest version. Some data may have been reset.");
        }
    }, []);

    // --- The providers are now rendered *after* the cleanup check has run ---
    return (
        <AuthProvider>
            <BlogProvider>
                <AppContent />
